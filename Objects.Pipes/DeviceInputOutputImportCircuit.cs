...
public class DeviceInputOutputImportCircuit : DeviceInputOutputImport, ICircuitHolder, IDensePoolable
{
	[SerializeField]
	...
	public override void OnChildEnterInventory(DynamicThing newChild)
	{
		...
		RefreshError();
		if (base.InteractOpen.State == 0)
		{
			WaitSetVisibility(isVisible: false).Forget();
		}
	...
	public void Execute()
	{
		if (GameManager.RunSimulation && !IsCursor && OnOff && Powered && GameManager.GameState == GameState.Running && !WorldManager.IsGamePaused && GameManager.GameState != GameState.None && (bool)ProgrammableChip && !ProgrammableChip.CompilationError)
		{
	...
	public override ThingSaveData SerializeSave()
	{
		...
		ThingSaveData result = (savedData = new DeviceInputOutputImportCircuitSaveData());
	...
	public override void DeserializeSave(ThingSaveData savedData)
	{
		...
		if (savedData is DeviceInputOutputImportCircuitSaveData { DeviceIDs: not null } deviceInputOutputImportCircuitSaveData)
		{
			for (int i = 0; i < _DeviceIDs.Length; i++)
			{
				_DeviceIDs[i] = deviceInputOutputImportCircuitSaveData.DeviceIDs[i];
			}
		}
	}
	...
	protected override void InitialiseSaveData(ref ThingSaveData savedData)
	{
		...
		if (savedData is DeviceInputOutputImportCircuitSaveData deviceInputOutputImportCircuitSaveData)
		{
			deviceInputOutputImportCircuitSaveData.DeviceIDs = new long[Devices.Length];
			for (int i = 0; i < Devices.Length; i++)
			{
				deviceInputOutputImportCircuitSaveData.DeviceIDs[i] = ((Devices[i] == null) ? 0 : Devices[i].ReferenceId);
			}
		}
	}
}
