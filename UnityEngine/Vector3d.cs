...
public struct Vector3d
{
	...
	public static Vector3d operator *(Quaternion rotation, Vector3d value)
	{
		...
		double num13 = value.x * (1.0 - (double)num10 - (double)num12) + value.y * (double)(num8 - num6) + value.z * (double)(num9 + num5);
		double num14 = value.x * (double)(num8 + num6) + value.y * (1.0 - (double)num7 - (double)num12) + value.z * (double)(num11 - num4);
		double num15 = value.x * (double)(num9 - num5) + value.y * (double)(num11 + num4) + value.z * (1.0 - (double)num7 - (double)num10);
	...
	{
		return new Vector3d(Vector3.Slerp((Vector3)from, (Vector3)to, (float)t));
	}
	...
	{
		return new Vector3d(Vector3.RotateTowards((Vector3)current, (Vector3)target, (float)maxRadiansDelta, (float)maxMagnitudeDelta));
	}
	...
	public static double Dot(Vector3 lhs, Vector3d rhs)
	{
	...
	public static Vector3d MultiplyPoint3x4(Matrix4x4 matrix, Vector3d point)
	{
		Vector3d result = default(Vector3d);
		result.x = (float)((double)matrix.m00 * point.x + (double)matrix.m01 * point.y + (double)matrix.m02 * point.z) + matrix.m03;
		result.y = (float)((double)matrix.m10 * point.x + (double)matrix.m11 * point.y + (double)matrix.m12 * point.z) + matrix.m13;
		result.z = (float)((double)matrix.m20 * point.x + (double)matrix.m21 * point.y + (double)matrix.m22 * point.z) + matrix.m23;
		return result;
	}
	...
	{
		return Math.Acos(Math.Clamp(Dot(from.normalized, to.normalized), -1.0, 1.0)) * 57.29578;
	}
	...
	{
		return Math.Acos(Math.Clamp(Dot(from.normalized, to.normalized), -1.0, 1.0));
	}
}
