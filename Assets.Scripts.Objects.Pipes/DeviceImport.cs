...
public class DeviceImport : Device, ISmartRotatable
{
	...
	private bool IsCurrentImportAnimComplete { get; set; }
	...
	{
		ThingSaveData savedData = new DeviceImportSaveData();
	...
	public override void DeserializeSave(ThingSaveData savedData)
	{
		...
		if (savedData is DeviceImportSaveData deviceImportSaveData)
		{
			_importCount = deviceImportSaveData.ImportCount;
			SetImportState(deviceImportSaveData.ImportState);
	...
	protected override void InitialiseSaveData(ref ThingSaveData savedData)
	{
		...
		if (savedData is DeviceImportSaveData deviceImportSaveData)
		{
			deviceImportSaveData.ImportCount = _importCount;
			deviceImportSaveData.ImportState = _importState;
		}
	}
	...
	private void SetImportState(int value)
	{
	...
	{
		if (SkipCollectingWhenHasImportChute && (bool)ImportChute)
		{
			return;
		}
		Item item = dynamicThing as Item;
		if ((bool)item && !ImportQueue.Contains(dynamicThing) && (ImportSlot.Type == Slot.Class.None || item.SlotType == ImportSlot.Type))
		{
	...
	public override void OnChildExitInventory(DynamicThing previousChild)
	{
		...
		if (ImportingThing == previousChild)
		{
			if (GameManager.RunSimulation && CanCompleteImport)
			{
