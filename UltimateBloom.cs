...
public class UltimateBloom : MonoBehaviour
{
	...
	{
		return Mathf.Max(Mathf.Max(Mathf.Max(m_DownscaleCount, m_UseAnamorphicFlare ? m_AnamorphicDownscaleCount : 0), m_UseLensFlare ? (GetGhostBokehLayer() + 1) : 0), m_UseStarFlare ? m_StarDownscaleCount : 0);
	}
	...
	{
		bool flag = false;
		flag = ((m_HDR != HDRBloomMode.Auto) ? (m_HDR == HDRBloomMode.On) : (source.format == RenderTextureFormat.ARGBHalf && m_Cam.allowHDR));
			...
			if ((!(sources[num6] == null) || !m_DirectUpsample) && (array2[num6] || !m_DirectUpsample))
			{
				...
				if (!array2[num6])
				{
					if (renderTexture != null)
					{
						if (flag)
						{
							GaussianBlur1(renderTexture, renderTexture2, (m_AnamorphicDirection == AnamorphicDirection.Vertical) ? num7 : 0f, (m_AnamorphicDirection == AnamorphicDirection.Horizontal) ? num8 : 0f, null, BlurSampleCount.FourSimple, Color.white, 1f);
						}
						else
						{
				...
				else
				{
					...
					if (flag && renderTexture != null)
					{
						...
						GaussianBlur1(renderTexture, renderTexture3, (m_AnamorphicDirection == AnamorphicDirection.Vertical) ? num7 : 0f, (m_AnamorphicDirection == AnamorphicDirection.Horizontal) ? num8 : 0f, null, BlurSampleCount.FourSimple, Color.white, 1f);
	...
	private void CachedUpsample(RenderTexture[] sources, RenderTexture[] destinations, int originalWidth, int originalHeight, BlurSampleCount upsamplingCount)
	{
		...
		for (int num = destinations.Length - 1; num >= 0; num--)
		{
			...
			{
				int num2 = (int)Mathf.Pow(2f, num);
				if (num2 == 0)
				{
					Debug.Log("System.DivideByZeroException: Division by zero");
					continue;
				}
				m_UpSamples[num] = RenderTexture.GetTemporary(originalWidth / num2, originalHeight / num2, 0, m_Format);
				m_UpSamples[num].filterMode = FilterMode.Bilinear;
			}
			float num3 = 1f;
			if (m_BloomUsages[num])
			{
				float num4 = 1f / (float)sources[num].width;
				float verticalBlur = 1f / (float)sources[num].height;
				GaussianBlurSeparate(m_DownSamples[num], m_UpSamples[num], num4 * num3, verticalBlur, renderTexture, upsamplingCount, m_BloomColors[num], m_BloomIntensities[num]);
			}
