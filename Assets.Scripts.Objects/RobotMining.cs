...
public class RobotMining : WheeledBase, IBatteryPowered, IPowered, IDensePoolable, IReferencable, IEvaluable, ICircuitHolder, IMiningTool, ITransmitable, ILogicable, IRepairable, IGenerateMinables, ITrackable
{
	...
	private readonly List<TargetMinableData> _minableDataQueue = new List<TargetMinableData>();

	private TargetMinableData? TargetMinable;
	...
	private float _minableScanTimeout;
	...
	private static readonly int maxMiningDepth = 3;

	private int _codeErrorState;

	private float _lastIsStuckCheckTime;

	private Vector3 _lastCheckPos;

	private static readonly float _isStuckCheckAmount = 60f;

	private static readonly float _isStuckMovementAmount = 0.1f;
	...
	private static readonly int MinableSearchArea = 16;
	...
	{
		if (Time.time - _lastIsStuckCheckTime > _isStuckCheckAmount)
		{
			if ((base.ThingTransformPosition - _lastCheckPos).magnitude < _isStuckMovementAmount)
			{
				Unstuck();
			}
			_lastCheckPos = base.ThingTransformPosition;
	...
	private void Unstuck()
	{
		...
		Vector2 vector = new Vector2((float)Math.Sin(num * _degreesToRadians), (float)Math.Cos(num * _degreesToRadians)) * MathF.Sqrt(UnityEngine.Random.Range(0, 1) * 2);
		Vector3 thingTransformPosition = base.ThingTransformPosition;
		thingTransformPosition.x += vector.x;
		thingTransformPosition.z += vector.y;
		thingTransformPosition.y = SpawnPoint.GetSafePoint(thingTransformPosition).y + 0.5f;
	...
	private async UniTaskVoid Mine(CancellationToken cancellationToken)
	{
		base.TargetMotorPower = 0f;
		base.TargetBrakePower = Power;
		OnServer.Interact(base.InteractImport, 1);
		await UniTask.Delay(3600, ignoreTimeScale: false, PlayerLoopTiming.Update, cancellationToken);
		if (GameManager.RunSimulation && TargetMinable.HasValue && TargetMinable.Value.Vein.GetActive(TargetMinable.Value.MinableIndex))
		{
			TargetMinable.Value.Vein.TryMineServer(TargetMinable.Value.Vein.GetMinableWorldPosition(TargetMinable.Value.MinableIndex).FloorToInt(), out Ore createdOre, position);
			List<TargetMinableData> minableDataQueue = _minableDataQueue;
			Vein vein = minableDataQueue[minableDataQueue.Count - 1].Vein;
			List<TargetMinableData> minableDataQueue2 = _minableDataQueue;
			vein.RemoveAimeeMinableHash(minableDataQueue2[minableDataQueue2.Count - 1].MinableIndex);
	...
	public void ClearMinableQueue()
	{
		for (int i = 0; i < _minableDataQueue.Count; i++)
		{
			Vein.AllAimeeQueuedMinables.Remove(_minableDataQueue[i].Vein.MakeMinableHash(_minableDataQueue[i].MinableIndex));
		}
		_minableDataQueue.Clear();
	}

	private bool MovingToMineable()
	{
		if (!TargetMinable.HasValue)
		{
			return false;
		}
		if (_mineCancellation.Initialized)
		{
			return true;
		}
		Vector3 minableWorldPosition = TargetMinable.Value.Vein.GetMinableWorldPosition(TargetMinable.Value.MinableIndex);
		_ = minableWorldPosition + Vector3.up * 0.5f - Lense.position;
		Vector3 vector = minableWorldPosition - base.ThingTransformPosition;
		int num = maxMiningDepth + 1;
		if (vector.sqrMagnitude <= (float)(num * num))
		{
			if (!TargetMinable.Value.Vein.GetActive(TargetMinable.Value.MinableIndex))
			{
				List<TargetMinableData> minableDataQueue = _minableDataQueue;
				Vein vein = minableDataQueue[minableDataQueue.Count - 1].Vein;
				List<TargetMinableData> minableDataQueue2 = _minableDataQueue;
			...
			_mineCancellation.CancelAndInitialize();
			Mine(_mineCancellation.Token).Forget();
			return true;
		}
		MoveTowards(minableWorldPosition, num);
		return true;
	}

	private void PathToTarget()
	{
		if (!IsBusy && InvalidPath)
		{
			PathList = null;
			_currentPathTarget = TargetPosition.ToGrid();
			RegisterPathFindingTask(_currentPathTarget);
			return;
		}
		Grid3 grid = base.GridController.WorldToLocalGrid(CenterPosition);
		if (PathList != null && PathList.Count > 0)
		{
			Vector3 force = Vector3.zero;
			TargetGrid = base.GridController.LocalToWorld(PathList[0].Grid);
			AimVector = TargetGrid - base.ThingTransformPosition;
			if (Mathf.Abs(Vector3.Dot(Vector3.up, (PathList[0].Grid - base.ThingTransformPosition.ToGrid()).ToVector3().normalized)) > 0.9f && PathList.Count > 1)
			{
				PathList.RemoveAt(0);
			}
			...
			{
				force = Vector3.up * UnityEngine.Random.Range(0.90000004f, 5f);
				if (PathList.Count > 0)
				{
					PathList.RemoveAt(0);
				}
				if (PathList.Count > 0 && PathList[0].Grid == grid + Grid3.Up)
				{
					PathList.RemoveAt(0);
			...
			if (PathList.Count > 0 && PathList[0].Grid == grid)
			{
				PathList.RemoveAt(0);
				LastPathChange = Time.fixedTime;
			}
			...
			{
				MoveTowards(CurrentTargetGrid.ToVector3());
			}
		}
		if (PathList == null || PathList.Count == 0)
		{
	...
	private void Roam()
	{
		...
		if (TargetMinable.HasValue && TargetMinable.Value.Vein.TryMineServer(TargetMinable.Value.Vein.GetMinableWorldPosition(TargetMinable.Value.MinableIndex).FloorToInt(), out Ore createdOre, position))
		{
			OnMinedOre(createdOre);
	...
	private void OnDrawGizmos()
	{
		...
		if (PathList != null)
		{
			Vector3 vector = GridPosition.ToVector3();
			for (int i = 0; i < PathList.Count; i++)
			{
				GridPathfinder.NpcPathGrid npcPathGrid = PathList[i];
				Gizmos.DrawSphere(npcPathGrid.Grid.ToVector3(), 0.5f);
				Gizmos.DrawLine(vector, npcPathGrid.Grid.ToVector3());
				vector = npcPathGrid.Grid.ToVector3();
			}
		}
		if (TargetMinable.HasValue)
		{
			Vein.GetVeinAtPosition(position);
			Gizmos.DrawCube(TargetMinable.Value.Vein.GetMinableWorldPosition(TargetMinable.Value.MinableIndex), Vector3.one);
		}
	}
			...
			if (!IsStorageEmpty)
			{
				foreach (Slot storageSlot in _storageSlots)
				{
					if ((bool)storageSlot.Occupant)
					{
						...
						text = ((!(storageSlot.Occupant is IQuantity quantity)) ? (text + $"{storageSlot.Occupant.ToTooltip()}") : (text + string.Format("{1} x {0}", storageSlot.Occupant.ToTooltip(), ExtensionMethods.ToString(quantity.GetQuantity, "yellow"))));
					}
				}
			}
	...
	public virtual void RegisterPathFindingTask(Grid3 localGrid, bool force = false)
	{
		if (force || PathList == null || PathList.Count <= 0)
		{
			GridController controller = GridController.GetController(base.Position);
			Vector3 vector = base.Position + Vector3.up * 0.5f;
			Vector3 vector2 = controller.LocalToWorld(localGrid);
			RoomManager.RegisterNewTask(this, controller.WorldToLocalGrid(vector), localGrid);
			Debug.DrawRay(vector, vector2 - vector, Color.green, 10f);
			LastPathChange = GameManager.FixedTime;
			if (!IsBusy)
			{
				IsBusy = true;
				StationaryTolerance = 5f;
				LastPathChange = GameManager.FixedTime;
			}
		}
	}
