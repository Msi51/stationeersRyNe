...
public class SSAOPro : MonoBehaviour
{
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		...
		RenderTexture temporary3 = RenderTexture.GetTemporary(source.width / num2, source.height / num2, 0, RenderTextureFormat.ARGB32);
		RenderTexture temporary4 = RenderTexture.GetTemporary(source.width / Downsampling, source.height / Downsampling, 0, RenderTextureFormat.ARGB32);
		Graphics.Blit(temporary2, temporary3, Material, 0);
		Graphics.Blit(source, temporary3, Material, num);
		...
		for (int i = 0; i < BlurPasses; i++)
		{
			...
			Graphics.Blit(temporary3, temporary4, Material, (int)pass);
			temporary3.DiscardContents();
			Material.SetVector("_Direction", new Vector2(0f, 1f / (float)source.height));
			Graphics.Blit(temporary4, temporary3, Material, (int)pass);
			temporary4.DiscardContents();
		}
		if (!DebugAO)
		{
			Material.SetTexture("_SSAOTex", temporary3);
			Graphics.Blit(source, destination, Material, 7);
		}
		else
		{
