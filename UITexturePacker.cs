...
public class UITexturePacker
{
	...
	public static Rect[] PackTextures(Texture2D texture, Texture2D[] textures, int padding, int maxSize)
	{
		...
		for (int i = 0; i < textures.Length; i++)
		{
			if (textures[i] != null)
			{
				num += textures[i].width * textures[i].height;
			}
		}
		int num2 = 4;
		int num3 = 4;
		...
		if (num2 > maxSize || num3 > maxSize)
		{
			return null;
		}
		return PackTextures(texture, textures, num2, num3, padding, maxSize);
	}
	...
	public static Rect[] PackTextures(Texture2D texture, Texture2D[] textures, int width, int height, int padding, int maxSize)
	{
		...
		UITexturePacker uITexturePacker = new UITexturePacker(width, height, rotations: false);
		Storage[] array = new Storage[textures.Length];
		for (int i = 0; i < textures.Length; i++)
		{
			...
			{
				for (num3 = 1; num3 >= 0; num3--)
				{
					rect = uITexturePacker.Insert(texture2D.width + num2 * padding, texture2D.height + num3 * padding, FreeRectChoiceHeuristic.RectBestAreaFit);
					...
					if (num2 == 0 && num3 == 0)
					{
						return PackTextures(texture, textures, width * ((width > height) ? 1 : 2), height * ((height >= width) ? 1 : 2), padding, maxSize);
					}
				}
				if (rect.width != 0f && rect.height != 0f)
				{
			...
			array[i] = default(Storage);
			array[i].rect = rect;
			array[i].paddingX = num2 != 0;
			array[i].paddingY = num3 != 0;
		}
		...
		for (int j = 0; j < textures.Length; j++)
		{
			...
			Rect rect2 = array[j].rect;
			int num4 = (array[j].paddingX ? padding : 0);
			int num5 = (array[j].paddingY ? padding : 0);
			Color[] array3 = texture2D2.GetPixels();
			if (rect2.width != (float)(texture2D2.width + num4))
			{
				Color[] pixels = texture2D2.GetPixels();
				for (int k = 0; (float)k < rect2.width; k++)
				{
					for (int l = 0; (float)l < rect2.height; l++)
					{
						int num6 = (int)rect2.height - (l + 1) + k * texture2D2.width;
						pixels[k + l * (int)rect2.width] = array3[num6];
					}
				}
				array3 = pixels;
			}
			texture.SetPixels((int)rect2.x, (int)rect2.y, (int)rect2.width - num4, (int)rect2.height - num5, array3);
			rect2.x /= width;
			rect2.y /= height;
			rect2.width = (rect2.width - (float)num4) / (float)width;
			rect2.height = (rect2.height - (float)num5) / (float)height;
			array2[j] = rect2;
		}
		texture.Apply();
		return array2;
